...
def main():
    ...
    if 'searched_role' in st.session_state and st.session_state.searched_role:
        ...
        - **Budgetnivå**: Välj mellan grundläggande, rekommenderad eller premium
        
        **Nyhet!** 🎉 Automatiska budgetrekommendationer baserat på historisk data!
        ...
        with col2:
                ...
                "1,450-1,860 SEK",
                "~105-135 klick",
                help="Högre CPC men nischad målgrupp"
            )
        with col3:
            st.metric(
                ...
                "2,030-2,100 SEK",
                "~23-24 klick",
                help="Specialiserad målgrupp"
            )
        
        # Budget tier comparison
        st.subheader("📊 Budgetnivåer förklarade")
        
        budget_comparison = pd.DataFrame({
            'Nivå': ['🥉 Grundläggande', '🥈 Rekommenderad', '🥇 Premium'],
            'Syfte': ['Basnivå för synlighet', 'Optimal balans', 'Maximal räckvidd'],
            'Framgångssannolikhet': ['60-70%', '70-85%', '85-95%'],
            'När att välja': ['Lång rekryteringstid OK', 'Standard rekrytering', 'Brådskande behov']
        })
        
        st.dataframe(budget_comparison, use_container_width=True, hide_index=True)
